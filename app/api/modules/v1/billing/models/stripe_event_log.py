...
class StripeEventLog(SQLModel, table=True):
    ...
    object_type: Optional[str] = Field(
        default=None, max_length=255, description="Stripe object type from event.data.object"
    )

    payload: Dict[str, Any] = Field(
        sa_column=Column(JSONB, nullable=False), description="Full Stripe webhook JSON payload"
    )
