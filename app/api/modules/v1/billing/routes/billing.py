...
async def create_or_get_billing_account(
    ...
    logger.info(
        "POST /billing - Create or get billing account",
    ...
    try:
        ...
        logger.error(
            "Failed to create/get billing account",
            exc_info=True,
...
@router.get("/", response_model=BillingSummaryResponse, summary="Get billing summary")
async def get_billing_summary(
    ...
    logger.info(
        "GET /billing - Fetch billing summary",
        extra={"org_id": str(org_id), "user_id": str(current_user.id)},
    )
...
async def create_portal_session(
    ...
    try:
        portal_session = await billing_service.create_portal_session(
            organization_id=org_id, return_url=request.return_url
        )
...
async def update_subscription(
    ...
    try:
        subscription = await billing_service.update_subscription(
            organization_id=org_id, new_plan=request.new_plan, prorate=request.prorate
        )
