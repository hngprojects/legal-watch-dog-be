...
class StripeClient:
    ...
    async def create_customer(
        ...
        logger.info(
            ...
            extra={"email": email, "name": name, "metadata": metadata},
        )

        try:
            customer = await asyncio.to_thread(
                stripe.Customer.create, email=email, name=name, metadata=metadata or {}
            )
    ...
    async def create_portal_session(
        ...
        logger.info(
            ...
            extra={"customer_id": customer_id, "return_url": return_url},
        )
    ...
    async def list_invoices(
        ...
        try:
            ...
            logger.info(
                ...
                extra={"customer_id": customer_id, "invoice_count": len(invoices.data)},
            )
    ...
    async def get_invoice_pdf(self, invoice_id: str) -> Optional[str]:
        """Get invoice PDF URL (async)"""
        logger.info("Retrieving invoice PDF", extra={"invoice_id": invoice_id})
        ...
        try:
            ...
            logger.info(
                ...
                extra={"invoice_id": invoice_id, "has_pdf": pdf_url is not None},
            )
    ...
    async def get_upcoming_invoice(
        ...
        try:
            ...
            invoice = await asyncio.to_thread(stripe.Invoice.upcoming, **params)

            logger.info(
                "Upcoming invoice retrieved",
                extra={"customer_id": customer_id, "amount_due": invoice.amount_due},
            )

            return invoice

        except stripe.error.InvalidRequestError:
            logger.info("No upcoming invoice found", extra={"customer_id": customer_id})
            return None
