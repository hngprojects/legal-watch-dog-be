...
def expire_trials():
    ...
    try:
        ...
        statement = select(BillingAccount).where(
            BillingAccount.trial_ends_at <= now, BillingAccount.status == "trialing"
        )
        ...
        logger.info("Trial expiration task completed", extra={"processed": len(expired_accounts)})
...
def _send_trial_expired_email(db: Session, billing_account: BillingAccount):
    ...
    try:
        ...
        admin_statement = select(User).where(
            User.organization_id == org.id, User.role.in_(["admin", "owner"])
        )
