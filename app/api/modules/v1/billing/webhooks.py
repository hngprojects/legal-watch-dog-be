...
async def handle_checkout_completed(event: stripe.Event, db: AsyncSession):
    ...
    logger.info(
        ...
        extra={"session_id": session.id, "customer": session.customer},
    )
    ...
    if not billing_account:
        logger.error(
            ...
            extra={"billing_account_id": billing_account_id},
        )
        return
    ...
    if subscription_id:
        ...
        logger.info(
            "Trial consumed immediately after successful payment",
            extra={
...
async def handle_invoice_payment_succeeded(event: stripe.Event, db: AsyncSession):
    ...
    logger.info(
        ...
        extra={"invoice_id": invoice.id, "customer": invoice.customer},
    )
    ...
    if not billing_account:
        logger.warning(
            "Billing account not found for customer",
            extra={"customer_id": invoice.customer},
        )
        return
    ...
    if billing_account.status == "trialing":
        ...
        logger.info(
            "Trial consumed on invoice payment",
            extra={
                "billing_account_id": str(billing_account.id),
