...
def build_change_details(old_obj: Any, new_obj: Any, fields: List[str]) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for field in fields:
        try:
            ...
            logger.warning("Failed to get change details for field '%s': %s", field, str(e), exc_info=True)
            continue

    return changes


def sanitize_sensitive_data(details: Dict[str, Any], sensitive_fields: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        sensitive_fields: List of fields to redact (default: ["password", "api_key", "token", "secret"])
