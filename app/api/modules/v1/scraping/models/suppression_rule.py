...
class SuppressionRule(SQLModel, table=True):
    ...
    rule_type: SuppressionRuleType = Field(description="Type of pattern matching to apply")
    rule_pattern: str = Field(
        max_length=500, description="Pattern to match (JSON path, regex, or field name)"
    )
    rule_description: str = Field(
        max_length=500, description="Human-readable explanation of what this rule suppresses"
    )
    created_by: UUID = Field(foreign_key="users.id")
    is_active: bool = Field(
        default=True, description="Whether this rule is currently being applied"
    )
