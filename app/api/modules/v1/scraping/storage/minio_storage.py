...
if _HAS_MINIO:
    ...
    logger.warning(
        "MinIO SDK not installed. Install `minio` package to enable storage operations: `pip install minio`"
    )
...
def upload_raw_content(file_data: bytes, bucket_name: str, object_name: str) -> str:
    ...
    if not _HAS_MINIO:
        raise Exception(
            "Missing optional dependency: `minio` package is not installed. Run `pip install minio` to enable storage operations."
        )
    if not minio_client:
...
def fetch_raw_content_from_minio(object_name: str, bucket_name: str = "raw-content") -> Optional[bytes]:
    """
