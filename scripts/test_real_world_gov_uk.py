...
async def test_real_world_extraction():
    ...
    try:
        ...
        print("[INFO] Initializing AIExtractionService...")
        ...
        try:
            ...
            print(f"[OK] Fetched content via Playwright ({len(html_content)} characters)")
        except Exception as e:
            print(f"[WARN] Playwright fetch failed: {e}")
            print("[INFO] Attempting fallback with requests library...")
            try:
                import requests
                ...
                print(f"[OK] Fetched content via requests ({len(html_content)} characters)")
            except Exception as e2:
                print(f"[FAIL] Both fetch methods failed: {e2}")
                return False
        ...
        try:
            ...
            # Convert string back to bytes for clean_html_content (expects bytes)
            html_bytes = html_content.encode('utf-8') if isinstance(html_content, str) else html_content
            cleaned_text = clean_html_content(html_bytes)
