...
@pytest.mark.asyncio
async def test_token_rotation_on_refresh(pg_sync_session, pg_async_session):
    """Test that refresh token rotation works correctly"""
    session = pg_sync_session

    org = Organization(name="Refresh Org")
    session.add(org)
    session.flush()
    ...
    login_result = await authenticate_user(
        db=async_session, email="refresh@example.com", password="Password123"
    )
    ...
    refresh_result = await refresh_access_token(
        db=async_session, user_id=str(user.id), old_refresh_token=old_refresh_token
    )
    ...
    assert (
        ...
        or "revoked" in exc_info.value.detail.lower()
    )
